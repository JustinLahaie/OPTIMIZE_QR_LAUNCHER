...
public partial class Form1 : Form
{
    ...
    private void Form1_Resize(object sender, EventArgs e)
    {
        ...
        {
            // Use Task.Run to perform the tray minimization on a background thread
            Task.Run(() =>
            {
                // Small delay to allow UI to respond
                Thread.Sleep(100);
                
                // Invoke these operations on the UI thread
                this.Invoke(new Action(() =>
                {
                    ...
                    // Enter more aggressive idle mode on a separate thread to avoid blocking UI
                    if (!isIdleMode)
                    {
                        Task.Run(() => EnterIdleMode());
                    }
                }));
            });
        }
    ...
    private void EnterIdleMode()
    {
        ...
        try
        {
            ...
            // Perform garbage collection on a background thread
            Task.Run(() => ForceMemoryCollection());
        }
        catch (Exception ex)
        {
    ...
    private void ExitIdleMode()
    {
        ...
        try
        {
            ...
            // First mark as no longer in idle mode to prevent any further idle operations
            isIdleMode = false;
            ...
            Task.Run(() => {
                try
                {
                    ...
                    using (var proc = Process.GetCurrentProcess())
                    {
                        ...
                        if (_debugMode)
                        {
                            // Use Invoke to safely update UI
                            this.Invoke(new Action(() => {
